...
def run_demo():
    ...
    logger_color = FastLogger(
        name="demo_color", level="DEBUG", color_output=True, console_output=True
    )
    ...
    logger_json = FastLogger(
        name="demo_json", level="INFO", json_format=True, console_output=True
    )
    ...
    with FastLogger(
        ...
        logger_async.info(
            "This message was processed via a background QueueListener thread!"
        )
        logger_async.info(
            "It does not block your main event loop or thread for file/network I/O."
        )
        logger_async.warning(
            "Combining async_safe=True with color_output=True works perfectly."
        )
    ...
    print()
    print(
        "✅ All features demonstrated! Check the 'logs/' folder to see the generated log files."
    )
